#!/bin/bash
#SBATCH --job-name=stepwise_threshold_eval
#SBATCH --output=logs/stepwise_threshold_%j.out
#SBATCH --error=logs/stepwise_threshold_%j.err
#SBATCH --ntasks=1
...
echo "🚀 Starting Step-wise Uncertainty Evaluation WITH THRESHOLD FILTERING at $(date)"
...
OUTPUT_PATH="stepwise_threshold_eval/$USER/$SLURM_JOB_ID"

# Parameters for step-wise uncertainty WITH threshold filtering
